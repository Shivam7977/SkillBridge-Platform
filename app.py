...
# ── Resume PDF helper ────────────────────────────────────────────────────────
TEMPLATE_NAMES = {
    1:  "Classic Tech",    2:  "Minimal Pro",      3:  "Fresh Graduate",
    4:  "Creative Bold",   5:  "Data Scientist",    6:  "Full Stack Dev",
    7:  "Product Manager", 8:  "UI/UX Designer",    9:  "Marketing Pro",
    10: "Finance / CPA",   11: "Healthcare",        12: "Cybersecurity",
    13: "DevOps / Cloud",  14: "Freelancer",        15: "Executive",
    16: "Educator",        17: "Sales Pro",         18: "Startup Founder",
}

def parse_skills(value):
    """Convert any MongoDB skills format into a clean Python list."""
    if not value:
        return []
...
def profile():
    ...
    user_profile = {
        ...
        'github_avatar':       user_data.get('github_avatar', ''),
        'github_repos':        user_data.get('github_repos', []),
        'github_langs':        user_data.get('github_langs', []),
        'github_followers':    user_data.get('github_followers', 0),
        'github_public_repos': user_data.get('github_public_repos', 0),
...
def resume_builder():
    try:
        user_data = users_collection.find_one({'_id': ObjectId(current_user.id)})
        user_projects = list(projects_collection.find({'created_by_id': ObjectId(current_user.id)}).sort('created_at', -1))
...
def resume_pdf():
    try:
        ...
        if template_num < 1 or template_num > 18:
            template_num = 1
    except (ValueError, TypeError):
        template_num = 1
    ...
    # Fetch using current_user.id (Flask-Login) — NOT session username
    user_data = users_collection.find_one({'_id': ObjectId(current_user.id)}) or {}
    user_projects = list(projects_collection.find({'created_by_id': ObjectId(current_user.id)}).sort('created_at', -1))

    # Normalise all skills to proper Python lists
    ...
    # Normalise skills_needed on every project
...
def chatbot():
    try:
        ...
        # last 10 exchanges (20 turns) for context
        ...
        proj_summary = '\n'.join([
            f"  - {p.get('title','Untitled')}: {p.get('description','')[:120]}"
            for p in user_projects
        ]) or '  (No projects yet)'

        roadmap_summary = '\n'.join([
            f"  - {r.get('goal','Unknown goal')}"
...
@login_required
def chatbot_history():
    """Return saved chat history for the current user."""
    try:
        uid = ObjectId(current_user.id)
        chat_doc = chat_history_collection.find_one({'user_id': uid})
        messages = chat_doc.get('messages', []) if chat_doc else []
        # Convert for JSON — remove ObjectId/datetime issues
        result = [
...
def view_community(community_id):
    ...
    messages = list(community_messages_collection.find(
...
def request_to_join_community(community_id):
    ...
    communities_collection.update_one(
        {"_id": obj_id},
        {"$addToSet": {"pending_requests": user_id}, "$pull": {"rejected_requests": user_id}}
    )
...
def approve_member(community_id, user_id):
    ...
    communities_collection.update_one(
        ...
        {"$pull": {"pending_requests": ObjectId(user_id)},
         "$push": {"members": {"user_id": ObjectId(user_id), "role": "member"}}}
    )
...
def decline_member(community_id, user_id):
    ...
    communities_collection.update_one(
        {"_id": obj_id},
        {"$pull": {"pending_requests": user_obj}, "$addToSet": {"rejected_requests": user_obj}}
    )
...
def remove_member(community_id, user_id):
    ...
    communities_collection.update_one(
        {"_id": ObjectId(community_id)},
        {"$pull": {"members": ObjectId(user_id), "admins": ObjectId(user_id)}}
    )
...
def add_certificate():
    try:
        ...
        cert['added_at'] = cert['added_at'].isoformat()
        return jsonify({'success': True, 'certificate': cert})
    except Exception as e:
        print(f"Add certificate error: {e}")
...
def delete_certificate(cert_id):
    try:
        users_collection.update_one(
            {'_id': ObjectId(current_user.id)},
            {'$pull': {'certificates': {'_id': ObjectId(cert_id)}}}
        )
        return jsonify({'success': True})
    except Exception as e:
        return jsonify({'success': False, 'error': str(e)}), 500
...
def edit_certificate(cert_id):
    try:
        ...
        users_collection.update_one(
            ...
            {'$set': {
                'certificates.$.name':          data.get('name', '').strip(),
                'certificates.$.issuer':        data.get('issuer', '').strip(),
                'certificates.$.issue_date':    data.get('issue_date', '').strip(),
                'certificates.$.expiry_date':   data.get('expiry_date', '').strip(),
                'certificates.$.credential_id': data.get('credential_id', '').strip(),
                'certificates.$.cert_url':      data.get('cert_url', '').strip(),
            }}
        )
        return jsonify({'success': True})
    except Exception as e:
...
def github_sync():
    try:
        ...
        update = {
            ...
            'github_avatar':       gh_data.get('avatar', ''),
            'github_repos':        gh_data.get('repos', []),
            'github_langs':        gh_data.get('top_languages', []),
            'github_followers':    gh_data.get('followers', 0),
            'github_public_repos': gh_data.get('public_repos', 0),
            'github_synced_at':    datetime.utcnow(),
        }
        user_data = users_collection.find_one({'_id': ObjectId(current_user.id)}) or {}
        if not user_data.get('github_url') and gh_data.get('github_url'):
            update['github_url'] = gh_data['github_url']
        if not user_data.get('location') and gh_data.get('location'):
            update['location'] = gh_data['location']
        if not user_data.get('about_me') and gh_data.get('bio'):
            update['about_me'] = gh_data['bio']
